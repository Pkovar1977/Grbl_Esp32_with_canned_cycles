...
static int cached_tool_number = 0;
...
Error gc_execute_line(char* line, uint8_t client) {
    static int last_tool = 0;
    
    // 1. Handle M998
    if (strstr(line, "M998") != NULL) {
        grbl_msg_sendf(client, MsgLevel::Info, "Tool change");
        return Error::Ok;  // Skip M998 completely
    }

    // 2. Extract tool number from ANY line with T
    if (strstr(line, "T") != NULL) {
        char* t_pos = strstr(line, "T");
        if (t_pos) {
            last_tool = atoi(t_pos + 1);
            grbl_msg_sendf(client, MsgLevel::Info, "Tool No: %d in Use", last_tool);
        }
    }

    // 3. Clean tool change lines (T...M6 → M6)
    if ( strstr(line, "M6") != NULL) {
        strcpy(line, "M6");
    }
...
case 73:  // G73 - chip-breaking peck cycle
    axis_command = AxisCommand::MotionMode;
    gc_block.modal.motion = Motion::G73;
    
    // PRESET Q if not specified in this block
    if (!(value_words & bit(GCodeWord::Q))) {
        gc_block.values.q = 0.1f;  // Default peck depth
        grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, 
                       "G73: No Q specified, using default Q=0.1mm");
    }
    mg_word_bit = ModalGroup::MG1;
    break;
                    case 80:  // G80 - cancel canned cycle
                        ...
                        mg_word_bit           = ModalGroup::MG1;
                        break;
                    case 81:  // G81 - simple drilling canned cycle
                        axis_command          = AxisCommand::MotionMode;
                        gc_block.modal.motion = Motion::G81;
                        mg_word_bit           = ModalGroup::MG1;
                        break;
                    case 82:  // G82 - drilling with dwell
                        axis_command          = AxisCommand::MotionMode;
                        gc_block.modal.motion = Motion::G82;
                        mg_word_bit           = ModalGroup::MG1;
                        break;
case 83:  // G83 - full-retract peck cycle  
    axis_command = AxisCommand::MotionMode;
    gc_block.modal.motion = Motion::G83;
    
    // PRESET Q if not specified in this block
    if (!(value_words & bit(GCodeWord::Q))) {
        gc_block.values.q = 0.1f;  // Default peck depth
        grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, 
                       "G83: No Q specified, using default Q=0.1mm");
    }
    mg_word_bit = ModalGroup::MG1;
    break;
                    case 17:
...
case 6:  // M6
{
    ...
    // Also keep original M6 behavior
    ...
    // Handle modal canned cycle parameters
    if (gc_state.modal.motion == Motion::G81 || gc_state.modal.motion == Motion::G82 ||
        gc_state.modal.motion == Motion::G83 || gc_state.modal.motion == Motion::G73) {
        if (!bit_istrue(value_words, bit(GCodeWord::R))) {
            gc_block.values.r = gc_state.canned_r;
        } else {
            gc_state.canned_r = gc_block.values.r;
        }
        if (!bit_istrue(value_words, bit(GCodeWord::Z))) {
            gc_block.values.xyz[Z_AXIS] = gc_state.canned_z;
        } else {
            gc_state.canned_z = gc_block.values.xyz[Z_AXIS];
        }
        if (!bit_istrue(value_words, bit(GCodeWord::Q))) {
            gc_block.values.q = gc_state.canned_q;
        } else {
            gc_state.canned_q = gc_block.values.q;
        }
        if (!bit_istrue(value_words, bit(GCodeWord::P))) {
            gc_block.values.p = gc_state.canned_p;
        } else {
            gc_state.canned_p = gc_block.values.p;
        }
    }
    ...
    */
        ...
        if (gc_block.modal.motion == Motion::Seek) {
            ...
            switch (gc_block.modal.motion) {
                ...
                case Motion::G83:
                    ...
                    break;
                case Motion::G73:
                    // G73 chip-breaking peck cycle (treated like G83 where applicable)
                    break;
    ...
    // Canned cycles (G81,G82,G83,G73) use R (retract), P (dwell), Q (peck increment), and L (repeat)
    if ((gc_block.modal.motion == Motion::G81) || (gc_block.modal.motion == Motion::G82) ||
        ...
        bit_false(value_words, (bit(GCodeWord::R) | bit(GCodeWord::P) | bit(GCodeWord::Q) | bit(GCodeWord::L)));
    }
    if (value_words) {
    ...
    */
    ...
    if (gc_state.modal.motion != Motion::None) {
        if (axis_command == AxisCommand::MotionMode) {
            ...
            } else if ((gc_state.modal.motion == Motion::G81) || (gc_state.modal.motion == Motion::G82) ||
                       (gc_state.modal.motion == Motion::G83) || (gc_state.modal.motion == Motion::G73)) {
                // Execute canned drilling cycles (G81/G82/G83/G73)
                GCUpdatePos canned_update = canned_cycle_execute(&gc_block, pl_data);
                gc_update_pos = canned_update;
            } else {
