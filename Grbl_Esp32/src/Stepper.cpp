...
bool get_rotary_disable() {  // returns true if steppers are disabled
    bool disabled = false;
#ifdef ROTARY_DISABLE_PIN
    disabled = digitalRead(ROTARY_DISABLE_PIN);
#else
    return false;  // thery are never disabled if there is no pin defined
#endif
...
bool get_lubrication_disable() {  // returns true if steppers are disabled
    bool disabled = false;
#ifdef LUBRICATION_DISABLE_PIN
    disabled = digitalRead(LUBRICATION_DISABLE_PIN);
#else
    return false;  // thery are never disabled if there is no pin defined
#endif
