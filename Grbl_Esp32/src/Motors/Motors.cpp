...
#    endif
    ...
    if (ROTARY_DISABLE_PIN != UNDEFINED_PIN) {
        pinMode(ROTARY_DISABLE_PIN, OUTPUT);  // global motor enable pin
        grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "Global rotary disable pin:%s", pinName(ROTARY_DISABLE_PIN));
    }

    if (LUBRICATION_DISABLE_PIN != UNDEFINED_PIN) {
        pinMode(LUBRICATION_DISABLE_PIN, OUTPUT);  // global lubrication enable pin
        grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "Global lubrication disable pin:%s", pinName(LUBRICATION_DISABLE_PIN));
    }   
...
void motors_set_disable(bool disable, uint8_t mask) {
    ...
    digitalWrite(ROTARY_DISABLE_PIN, disable);
    digitalWrite(LUBRICATION_DISABLE_PIN, disable);
